    ...
    def get_local_fallback_response(self, message: str) -> str:
        ...
        if "explain https" in msg_lower or "how https works" in msg_lower or msg_lower == "https":
            return (
                ...
                "```mermaid\n"
                "flowchart TD\n"
                "    subgraph P1 [Phase 1: Handshake & Verification]\n"
                "        A[\"1. Client sends ClientHello\"] --> B[\"2. Server sends ServerHello & SSL Certificate\"]\n"
                "        B --> C[\"3. Client verifies Certificate with CA\"]\n"
                "    end\n"
                "    subgraph P2 [Phase 2: Key Exchange]\n"
                "        C --> D[\"4. Client encrypts Pre-Master Secret with Server Public Key\"]\n"
                "        D --> E[\"5. Server decrypts Pre-Master Secret using Private Key\"]\n"
                "        E --> F[\"6. Both generate matching Symmetric Session Keys\"]\n"
                "    end\n"
                "    subgraph P3 [Phase 3: Encrypted Data Exchange]\n"
                "        F --> G[\"7. Handshake Finished confirmation\"]\n"
                "        G --> H[\"8. All traffic encrypted with Session Key (HTTPS Active)\"]\n"
                "    end\n"
                "```\n\n"
                "#### Core Pillars:\n"
                "- **Asymmetric Encryption**: Used during initial handshake to securely exchange the session key.\n"
                "- **Symmetric Encryption**: Fast key used for all subsequent data transfer.\n"
                "- **Authentication**: CAs verify server identity to prevent spoofing."
            )
