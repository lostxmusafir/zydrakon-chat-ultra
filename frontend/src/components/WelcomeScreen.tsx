"use client";

import React, { useEffect, useState } from "react";
import { ZydrakonLogo } from "./ZydrakonLogo";
import { ArrowRight, User, ShieldCheck } from "lucide-react";

interface WelcomeScreenProps {
  onEnter: () => void;
  onLogin?: () => void;
}

export function WelcomeScreen({ onEnter, onLogin }: WelcomeScreenProps) {
  const [progress, setProgress] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          clearInterval(interval);
          return 100;
        }
        return prev + 5;
      });
    }, 80);

    return () => clearInterval(interval);
  }, []);

  return (
    <div className="fixed inset-0 z-50 bg-[#0a090b] text-white flex items-center justify-center p-4 md:p-8 overflow-y-auto select-none">
      {/* Background Ambient Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] bg-orange-600/10 blur-[140px] rounded-full pointer-events-none -z-10" />

      {/* UNIFIED SINGLE LARGE CARD CONTAINER (IMAGE 2 DESIGN) */}
      <div className="w-full max-w-5xl bg-[#141316] border border-zinc-800/80 rounded-[32px] overflow-hidden shadow-2xl grid grid-cols-1 lg:grid-cols-2 my-auto min-h-[580px]">
        
        {/* LEFT PANEL: Dark Theme Brand Showcase */}
        <div className="p-8 md:p-12 bg-[#0e0d10] flex flex-col justify-between relative overflow-hidden text-white min-h-[480px]">
          {/* Top Tagline */}
          <div className="flex items-center gap-2">
            <span className="text-xs text-zinc-400 font-medium tracking-wide">
              Global AI intelligence made simple — streaming solutions for you.
            </span>
          </div>

          {/* Center Headline & Dragon Artwork */}
          <div className="my-auto space-y-8 py-6">
            <div className="relative inline-block cursor-pointer" onClick={onEnter}>
              <div className="absolute -inset-4 rounded-full bg-orange-500/20 blur-2xl pointer-events-none" />
              <ZydrakonLogo size={140} className="relative drop-shadow-[0_0_35px_rgba(249,115,22,0.4)] transition-transform hover:scale-105 duration-300" />
            </div>

            <div className="space-y-3">
              <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white leading-tight">
                Unleash Your <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-amber-400 to-yellow-300">
                  Intelligence.
                </span>
              </h1>
              <p className="text-xs text-zinc-400 font-medium max-w-sm leading-relaxed">
                Next-Generation Multi-LLM Round-Robin Engine with instant streaming and Bcrypt security.
              </p>
            </div>
          </div>

          {/* Left Bottom Footer */}
          <div className="text-[11px] text-zinc-500 font-mono flex items-center justify-between pt-4 border-t border-zinc-800/60">
            <span>© 2026 Zydrakon AI</span>
            <span>Developed by <strong className="text-zinc-300">Raj Patil</strong></span>
          </div>
        </div>

        {/* RIGHT PANEL: Clean High-Contrast Action Card (Image 2 style) */}
        <div className="p-8 md:p-12 bg-white text-zinc-900 rounded-[28px] lg:m-2 flex flex-col justify-between shadow-2xl relative overflow-hidden text-center min-h-[480px]">
          
          {/* Top Header */}
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-2.5">
              <ZydrakonLogo size={36} />
              <span className="font-extrabold text-base tracking-wide text-zinc-900">Zydrakon AI</span>
            </div>
            <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">
              ● v2.5 Engine Active
            </span>
          </div>

          {/* Center Action */}
          <div className="my-auto space-y-6 py-6 w-full">
            <ZydrakonLogo size={70} className="mx-auto" />

            <div>
              <h2 className="text-3xl md:text-4xl font-extrabold text-zinc-900 tracking-tight">
                Welcome
              </h2>
              <p className="text-xs text-zinc-500 mt-1">
                Access your Zydrakon AI workspace or sign in to your account
              </p>
            </div>

            <div className="space-y-3 pt-2 max-w-sm mx-auto">
              <button
                onClick={onLogin || onEnter}
                className="w-full py-3.5 px-6 rounded-full bg-gradient-to-r from-orange-600 via-orange-500 to-amber-500 hover:from-orange-500 hover:to-amber-400 text-white font-bold text-sm tracking-wide shadow-lg shadow-orange-500/30 transition-all flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.01]"
              >
                <User className="w-4 h-4" />
                <span>Sign In to Account</span>
              </button>

              <button
                onClick={onEnter}
                className="w-full py-3.5 px-6 rounded-full bg-zinc-100 hover:bg-zinc-200/80 border border-zinc-200 text-zinc-900 font-bold text-sm tracking-wide transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
                <span>Launch Workspace as Guest</span>
                <ArrowRight className="w-4 h-4 text-orange-600" />
              </button>
            </div>
          </div>

          {/* Right Bottom Footer */}
          <div className="flex items-center justify-between text-xs text-zinc-400 font-medium pt-4 border-t border-zinc-100">
            <span>© 2005-2026 Zydrakon Inc.</span>
            <div className="flex items-center gap-4">
              <span className="hover:text-zinc-600 cursor-pointer">Contact Us</span>
              <span className="hover:text-zinc-600 cursor-pointer">English ⌄</span>
            </div>
          </div>
        </div>

      </div>
    </div>
  );
}


